...
class Market:
    ...
    @classmethod
    def new(cls, platform: str, market_id: str, title: str, description: str) -> 'Market':
        """创建新市场"""
        return cls(
            platform=platform,
            market_id=market_id,
    ...
    def with_resolution_date(self, date: datetime) -> 'Market':
        """设置解析日期"""
        self.resolution_date = date
        return self

    def with_category(self, category: str) -> 'Market':
        """设置类别"""
        self.category = category
        return self

    def with_tags(self, tags: List[str]) -> 'Market':
        """设置标签"""
        self.tags = tags
        return self

    def with_slug(self, slug: str) -> 'Market':
        """设置slug"""
        self.slug = slug
        return self

    def with_token_ids(self, token_ids: List[str]) -> 'Market':
        """设置token IDs"""
        self.token_ids = token_ids
        return self

    def with_outcome_prices(self, yes: float, no: float) -> 'Market':
        """设置结果价格"""
        self.outcome_prices = (yes, no)
        return self

    def with_market_data(self, best_ask: float, best_bid: float, last_trade: Optional[float]) -> 'Market':
        """设置市场数据"""
    ...
    def is_15m_crypto_market(self) -> bool:
        ...
        if self.slug_is_15m_crypto():
            return True
        ticker = self.slug if self.slug else self.market_id
    ...
    def coin_from_slug(self) -> Optional[str]:
        ...
        ticker = (self.slug if self.slug else self.market_id).lower()
